...
function cqInputOpen(payload) {
	...
	cqInputField.removeAttribute("maxlength");
	cqInputField.maxLength = 10000;

	// console.log("INPUT MAX:", cqInputField.maxLength);
...
function cqInputForceCancel() {
    ...
    post("striano-input:cancel", {}).catch(() => {});
}
...
// Se il campo perde focus e siamo usciti dal modal -> chiudi tutto
cqInputField?.addEventListener("blur", () => {
    setTimeout(() => {
        if (!cqInputWrap || cqInputWrap.classList.contains("hidden")) return;
        if (cqInputSubmitting) return;

        // Click su OK/Cancel? Siamo ancora nel modal, non fare nulla
        if (cqInputWrap.contains(document.activeElement)) return;

        cqInputForceCancel();
    }, 50);
});

// Se perde focus direttamente tutta la NUI
window.addEventListener("blur", () => {
    if (cqInputWrap && !cqInputWrap.classList.contains("hidden")) {
        cqInputForceCancel();
    }
});
