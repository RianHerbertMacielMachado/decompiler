    ...
    let element = $(`
            <div class="${data[i].disabled ? "stepperDisabled next-radius" : "stepper next-radius"}" id=${i} ${data[i].style ? `style="${data[i].style}"` : ""}>
                ...
                <div className="column">
                    <div class="header" id=${i}> ${data[i].mexBtn}</div>
                </div>
            </div>
            `
    );
...
function range_slider(data, i) {
    let element = $(`
            <div class="sliderButton" ${data[i].spacer ? "is-spacer" : ""}" id=${i} ${data[i].style ? `style="${data[i].style}"` : ""}>
				...
				${data[i].multiplier == -1200 ? ` <div class="header" id=${i}>${data[i].header} <output id='label${i}'> ${data[i].range.value} pz = 0☯ </output></div> ` : ""}
				${data[i].multiplier == -1201 ? ` <div class="header" id=${i}>${data[i].header} <output id='label${i}'> ${data[i].range.value} pz = +0☯ </output></div> ` : ""}
				${data[i].multiplier == -1100 ? ` <div class="header" id=${i}>${data[i].header} <output id='label${i}'> ${data[i].range.value} / ${data[i].range.max} </output></div> ` : ""}
				${data[i].multiplier == -1101 ? ` <div class="header" id=${i}>${data[i].header} <output id='label${i}'> ${data[i].range.value} / ${data[i].range.max} </output></div> ` : ""}
                ${data[i].multiplier == -1000 ? ` <div class="header" id=${i}>${data[i].header} <output id='label${i}'> ${data[i].range.value} / ${data[i].range.max} </output></div> ` : ""}
                ...
                <input type="range"
                    ...
                    oninput="Slider_Output(${i}, ${data[i].multiplier}, ${data[i].currency})"
                    style="${data[i].style}"
...
function Slider_Output(index, multiplier, currency) {
    ...
    setTimeout(() => {
        if (new Date().getTime() - ms >= $('#range' + index).data('updated')) {
			...
			if (multiplier == -1100) { // MODIFICA DEI COMPONENTI MODELLO CUSTOM PEDS
				$.post(`https://${GetParentResourceName()}/CambiaComponente`, JSON.stringify({ index:e_range.value, currency:currency }))
				i2.innerHTML = ' <b>' + e_range.value + ' / ' + e_range.max + '</b>';
			} else {
				if (multiplier == -1101) { // MODIFICA DEI COMPONENTI TEXTURE CUSTOM PEDS
					$.post(`https://${GetParentResourceName()}/CambiaComponenteTexture`, JSON.stringify({ index:e_range.value, currency:currency }))
					i2.innerHTML = ' <b>' + e_range.value + ' / ' + e_range.max + '</b>';
				}
			}
        }
...
function bar_search(data, i) {
    let element = $(`
            <div class="${data[i].disabled ? "searchbarDisabled" : "searchbar"} ${data[i].spacer ? "is-spacer" : ""}" id=${i} ${data[i].style ? `style="${data[i].style}"` : ""}>
                ...
                <input type="text" id="${data[i].disabled ? "searchDisabled" : "search"}" ${data[i].disabled ? "disabled" : ""} placeholder="Cerca un elemento...">
            </div>
            `
    );
    $('#buttons').append(element);
...
$(document).click(function (event) {
    ...
    if (($target.closest('.leave').length && $('.leave').is(":visible")) || ($target.closest('.pin').length && $('.pin').is(":visible")) || ($target.closest('.stepper').length && $('.stepper').is(":visible")) || ($target.closest('.button').length && $('.button').is(":visible"))) {
        ...
        if (Button[id]) {
			...
			if (Button[id].resetNui) {
				$.post(`https://${GetParentResourceName()}/resetNui`)
			}
...
const CameraToggle = () => {
    $.post(`https://${GetParentResourceName()}/cameraToggle`)
}
...
window.addEventListener('keyup', funcname, false);
function funcname(e) {
	if (e.keyCode === 16) { CameraToggle(); }
	if (e.keyCode === 13) { $.post(`https://${GetParentResourceName()}/confermaPG`)	}
	if (e.keyCode === 46) { $.post(`https://${GetParentResourceName()}/closeMenuBase`) } // CANC
	if (e.keyCode === 27) { $.post(`https://${GetParentResourceName()}/closeMenuBase`) }
}
