$(function () {
	...
	function display(bool) {
		if (bool) {
		  $("#panel").show();
		  $("#striano_cards_input").val("");
		  $("#striano_cards_input").focus();
		} else {
		  $("#panel").hide();
		}
		isShowing = bool;
	...
	window.addEventListener("message", function (event) {
		...
		if (event.data.action == "range") {
			document.getElementById("rangeDisplay").innerHTML = event.data.status;
		}
		...
		if (event.data.action == "newNote") {
			...
			content = $(
				'<div class="container-desc" style="display: none; !important;"><div class="text">' +
				event.data.text +
				'</div></div>'
			);
		...
		if (event.data.action == "clearLogPoker") {
			for(let i=1 ; i<=10 ; i++) {
				$(".noteContainer").children().last().remove();
				totmex=0;
			}
	...
	$("#close").click(function () {
		$.post("https://striano_cards/exit", JSON.stringify({}));
		return;
	});
});
...
window.addEventListener('message', function(event) {
	...
	} else if (event.data.action == 'showTopMex') {
		let message = event.data.content;	
		$('#signTopMex').show();
		$('#messageTopMex').text(message);
	...
	} else if (event.data.action == 'hide_prova3') { $('#signTopMex').hide();
...
// ======================================================
// STRIANO MISSIONS! PRESO DA striano_missions
// ======================================================
let message = ''
let pocet = ''
let CurDispId = 0
